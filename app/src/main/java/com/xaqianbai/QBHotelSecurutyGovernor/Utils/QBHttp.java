...
public class QBHttp {
     ...
     * @param callback 回调方法
     */
    public static void request(final Context context, final OkHttpRequestBuilder builder, final QBCallback callback) {
            ...
            if (tokenManager.checkRefreshToken()) {
                ...
                refreshToken(context, new QBCallback() {
                    ...
                    public void doWork(Map<?, ?> map) {
                        SPUtils.put(context, "access_token", tokenManager.getAccessToken());//access_token
                        SPUtils.put(context, "refresh_token",tokenManager.getRefreshToken());//refresh_token
                    }
...
//                                autoLogin();
    ...
    private static void refreshToken(final Context context, final QBCallback callback) {
        OkHttpUtils.get()
                .url(HttpUrlUtils.getHttpUrl().getToken() +"/"+ tokenManager.getRefreshToken()
